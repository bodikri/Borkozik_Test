...
public class SharingService extends Service implements SharedPreferences.OnSharedPreferenceChangeListener {
    /**
     * GPS status code
     */
    ...
    private com.borkozic.location.ILocationRemoteService mBorkozicLocationService = null;
        ...
        executorThread.execute(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    //updateNotification(R.mipmap.ic_stat_sharing_in);//Процедурка да покаже че има размяна на информация при натрупване на много инфо пречи
    ...
    private byte[] getSituationBitmap(Situation situation) {
        ...
        b = Bitmap.createBitmap(width * 2, height * 2, Bitmap.Config.ARGB_8888);
    ...
    private void disconnect() {
        if (mBorkozicLocationService != null) {
            try {
                mBorkozicLocationService.unregisterCallback(mAndrozicLocationCallback);
            } catch (RemoteException e) {
                e.printStackTrace();
            }
            unbindService(mLocationConnection);
            mBorkozicLocationService = null;
        }
    }
    ...
    private BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals("com.borkozic.locatingStatusChanged")) {
                ...
                try {
                    isLocating = mBorkozicLocationService != null && mBorkozicLocationService.isLocating();
                } catch (RemoteException e) {
                    e.printStackTrace();
                }
                if (isLocating) {
    ...
    private ServiceConnection mLocationConnection = new ServiceConnection() {
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            mBorkozicLocationService = com.borkozic.location.ILocationRemoteService.Stub.asInterface(service);
            try {
                mBorkozicLocationService.registerCallback(mAndrozicLocationCallback);
                if (mBorkozicLocationService.isLocating())
                    doStart();
