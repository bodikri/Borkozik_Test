...
public class KmlFiles
{
	...
	public static List<Route> loadRoutesFromFile(File file) throws SAXException, IOException, ParserConfigurationException
	{
		...
		for (Track track : tracks)
		{
			...
			{
				String name = "RWPT"+i;
				route.addWaypoint(name, tp.latitude, tp.longitude, tp.elevation); //elev is added from me
				i++;
			}
			routes.add(route);
		}
	...
	public void endElement(String uri, String localName, String qName) throws SAXException
	{
		...
		else if (localName.equalsIgnoreCase(COORDINATES))
		{
			if (ispoint)
			{
				...
				waypoint.altitude = Double.parseDouble(coords[2].trim()); //Think here must be Altitude
			}
			if (istrack)
			{
