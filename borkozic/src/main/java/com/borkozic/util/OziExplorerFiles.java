...
public class OziExplorerFiles
{
	...
	public static Track loadTrackFromFile(final File file, final String charset, final long lines) throws IllegalArgumentException, IOException
	{
	    ...
	    while ((line = reader.readLine()) != null)
		{
			...
			double elevation = fields.length > 3 ? Double.parseDouble(fields[3]) * 0.3048: 0; // TODO Какво преобразуване се прави тук?
			if (fields.length >= 3)
	...
	public static List<Route> loadRoutesFromFile(final File file, final String charset) throws IOException, IllegalArgumentException
	{
		...
		if ("H1".equals(fields[0]))
		{
			...
			*/
			...
			try
			{
					route.show = Boolean.parseBoolean(fields[2]);
			}
			catch (NumberFormatException e)
			{
			}
		    ...
		    while ((line = reader.readLine()) != null)
			{
				...
				if (! "W".equals(fields[0]))
					continue;
				route.addWaypoint(fields[1].replace((char) 209, ','), Double.parseDouble(fields[2]), Double.parseDouble(fields[3]), Double.parseDouble(fields[4]));//Fields[4] - for Altitude is added from me
		...
		else if ("OziExplorer Route File Version 1.0".equals(fields[0]))
		{
		    ...
		    while ((line = reader.readLine()) != null)
			{
				...
				else if ("W".equals(fields[0]))
				{
					...
					route.addWaypoint(new Waypoint(fields[4].replace((char) 209, ','), fields[13].replace((char) 209, ','), Double.parseDouble(fields[5]), Double.parseDouble(fields[6]),Double.parseDouble(fields[7])));
	 ...
	 *
	 * @param file valid <code>File</code> with area waypoints
	 * @return <code>List<Area></code> the list of areas
	 * @throws IOException on file read error
	 * @throws IllegalArgumentException if file format is not rt2 or rte
	 */
	public static List<Area> loadAreasFromFile(final File file, final String charset) throws IOException, IllegalArgumentException
	{
		...
		if ("H1".equals(fields[0]))
		{
			...
			*/
			...
			Area area = new Area();
			areas.add(area);
			area.name = fields[1].replace((char) 209, ',');
			try
			{
				area.show = Boolean.parseBoolean(fields[2]);
			}
			catch (NumberFormatException e)
			{
			}
	...
	public static void saveAreaToFile(final File file, final String charset, final Area area) throws IOException
	{
		BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file, false), charset));
