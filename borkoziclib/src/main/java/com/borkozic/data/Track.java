...
public class Track
{
	...
	public void addPoint(boolean continous, double lat, double lon, double elev, double speed, double bearing, double accuracy, long time)
	{
		...
		{
			distance += Geo.distance(lastTrackPoint.latitude, lastTrackPoint.longitude, lat, lon, lastTrackPoint.elevation, elev);
		}
		...
		synchronized (trackpoints)
		{
			if (maxPoints > 0 && trackpoints.size() > maxPoints)
			{
				...
				distance -= Geo.distance(fp.latitude, fp.longitude, sp.latitude, sp.longitude, fp.elevation, sp.elevation);
	...
	public void removePoint(int location) throws IndexOutOfBoundsException
	{
		synchronized (trackpoints)
		{
			...
			distance -= Geo.distance(pp.latitude, pp.longitude, cp.latitude, cp.longitude, pp.elevation, cp.elevation);
			if (! last)
			{
				TrackPoint np = trackpoints.get(location + 1);
				distance -= Geo.distance(cp.latitude, cp.longitude, np.latitude, np.longitude, cp.elevation, np.elevation);
				distance += Geo.distance(pp.latitude, pp.longitude, np.latitude, np.longitude, pp.elevation, np.elevation);
			}
