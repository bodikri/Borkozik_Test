...
//		return area.minLat < 85.051129 && area.maxLat > -85.047336;
	...
	@Override
	public boolean drawMap(float bearing, double[] loc, int[] lookAhead, int width, int height, boolean cropBorder, boolean drawBorder, Canvas c) throws OutOfMemoryError
	{
		...
		double t_per_x=((double) width/(double)TILE_WIDTH/2+.5)+.4;
		double t_per_y=(double)(height * 1.f / TILE_HEIGHT / 2 + .5f)+.7;
		...
		double latw=((double) lookAhead[1]/(double)TILE_HEIGHT);
		double x1,x2,x3,x4,y1,y2,y3,y4,step_y,step_x,sx,sy;
		double max_y, min_y;
		if(bearing>3&&bearing<=87) {
			x1= (t_per_x*Math.cos(Math.toRadians(bearing))+t_per_y*Math.sin(Math.toRadians(-bearing)));
			y1 = (t_per_y*Math.cos(Math.toRadians(bearing))+t_per_x*Math.sin(Math.toRadians(bearing)));
			x2= (t_per_x*Math.cos(Math.toRadians(bearing))-t_per_y*Math.sin(Math.toRadians(-bearing)));
			//y2 = (-t_per_y*Math.cos(Math.toRadians(bearing))+t_per_x*Math.sin(Math.toRadians(bearing)));
			x3= (-t_per_x*Math.cos(Math.toRadians(bearing))-t_per_y*Math.sin(Math.toRadians(-bearing)));
			y3 = (-t_per_y*Math.cos(Math.toRadians(bearing))-t_per_x*Math.sin(Math.toRadians(bearing)));
			x4= (-t_per_x*Math.cos(Math.toRadians(bearing))+t_per_y*Math.sin(Math.toRadians(-bearing)));
			...
			max_y=y4;
			min_y=y4+step_y;
			for (int fx = (int)Math.round(x4); fx <= (int)Math.round(x2); fx++) {
				for (int fy =(int)Math.round(min_y) ; fy <= (int)Math.round(max_y) ; fy++) {
					int jx = osm_x + kt+fx;
					int iy = osm_y + mt+fy;
					int tx = width / 2 - map_xy[0] + jx * TILE_WIDTH;  //int tx = txb + (j - c_min) * TILE_WIDTH;
					int ty = height / 2 - map_xy[1] + iy * TILE_HEIGHT; //int ty = tyb + (i - r_min) * TILE_HEIGHT;
					Bitmap tile = getTile(jx, iy);
			...
			int mt = (int)Math.round(latw*(1-Math.cos(Math.toRadians(bearing))));
			int kt = (int)Math.round(latw*Math.sin(Math.toRadians(bearing)));
			for (int fx = -tiles_per_x-1; fx <= tiles_per_x; fx++) {
				for (int fy =kt -tiles_per_y-1 ; fy <= tiles_per_y ; fy++) {
				int jx = osm_x + kt+fx;
				int iy = osm_y + mt+fy;
				int tx = width / 2 - map_xy[0] + jx * TILE_WIDTH;
				int ty = height / 2 - map_xy[1] + iy * TILE_HEIGHT;
				Bitmap tile = getTile(jx, iy);
					if (tile != null && !tile.isRecycled()) {
						c.drawBitmap(tile, tx, ty, null);
					}
			...
			y1 = (t_per_y*Math.cos(Math.toRadians(bearing))+t_per_x*Math.sin(Math.toRadians(bearing)));
			x2= (t_per_x*Math.cos(Math.toRadians(bearing))-t_per_y*Math.sin(Math.toRadians(-bearing)));
			y2 = ((-t_per_y-.3)*Math.cos(Math.toRadians(bearing))+t_per_x*Math.sin(Math.toRadians(bearing)));
			x3= (-t_per_x*Math.cos(Math.toRadians(bearing))-t_per_y*Math.sin(Math.toRadians(-bearing)));
			//y3 = (-t_per_y*Math.cos(Math.toRadians(bearing))-t_per_x*Math.sin(Math.toRadians(bearing)));
			x4= (-t_per_x*Math.cos(Math.toRadians(bearing))+t_per_y*Math.sin(Math.toRadians(-bearing)));
			y4 = ((t_per_y+.3)*Math.cos(Math.toRadians(bearing))-t_per_x*Math.sin(Math.toRadians(bearing)));
			...
			for (int fx = (int)Math.round(x1); fx <= (int)Math.round(x3); fx++) {
				for (int fy =(int)Math.round(min_y) ; fy <= (int)Math.round(max_y) ; fy++) {
					int jx = osm_x + kt+fx;
					int iy = osm_y + mt+fy;
					int tx = width / 2 - map_xy[0] + jx * TILE_WIDTH;  //int tx = txb + (j - c_min) * TILE_WIDTH;
			...
			int mt = (int)Math.round(latw*(1-Math.cos(Math.toRadians(bearing))));
			int kt = (int)Math.round(latw*Math.sin(Math.toRadians(bearing)));
			for (int fx = -tiles_per_x; fx <= tiles_per_x; fx++) {
				for (int fy =-tiles_per_y ; fy <= tiles_per_y ; fy++) {
					int jx = osm_x + kt+fx;
					int iy = osm_y + mt+fy;
					int tx = width / 2 - map_xy[0] + jx * TILE_WIDTH;  //int tx = txb + (j - c_min) * TILE_WIDTH;
					int ty = height / 2 - map_xy[1] + iy * TILE_HEIGHT; //int ty = tyb + (i - r_min) * TILE_HEIGHT;
			...
			for (int fx = (int)Math.round(x2); fx <= (int)Math.round(x4); fx++) {
				for (int fy =(int)Math.round(min_y) ; fy <= (int)Math.round(max_y) ; fy++) {
					int jx = osm_x + kt+fx;
					int iy = osm_y + mt+fy;
					int tx = width / 2 - map_xy[0] + jx * TILE_WIDTH;  //int tx = txb + (j - c_min) * TILE_WIDTH;
			...
			for (int fx = (int)Math.round(x3); fx <= (int)Math.round(x1); fx++) {
				for (int fy =(int)Math.round(min_y) ; fy <= (int)Math.round(max_y) ; fy++) {
			...
			for (int ic = -tiles_per_x; ic <= tiles_per_x; ic++) {
				for (int jr = -tiles_per_y ; jr <= tiles_per_y ; jr++) {
