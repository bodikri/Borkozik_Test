...
public class StringFormatter
{
	...
	public static final String timeHSec(int seconds)
	{
		String[] time = timeSec(seconds);
		return time[0] + " " + time[1];
	}
	 ...
	 * "> 24 h" - for period more than 1 day
	 *
	 * @param seconds time in minutes
	 * @return Time period
	 */
	public static final String[] timeSec(int seconds)
	{
		int hour = 0;
		int min = 0;
		int sec = seconds;
		...
		if (sec <= 3599)
		{
			min = (int) Math.floor(seconds / 60); //взема цялата част от делнето
			sec = sec - min * 60;
			return new String[]{timeFormat.format(min) + ":" + timeFormat.format(sec), secondAbbr}; //min:sec
		}
		if (sec > 3599)
		{
